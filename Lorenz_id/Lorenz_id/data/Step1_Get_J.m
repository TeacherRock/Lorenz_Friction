clc; clear; close all;
latexArg = {'Interpreter','latex'};
legendArg = [latexArg(:)',{'FontSize'},{12}];
titleArg = [latexArg(:)', {'FontSize'},{16},{'FontWeight'},{'bold'}];

%% Auto fitting Version
FileNum = 1;
delta_tau = zeros(FileNum, 1);
% OL = [2];     % Omit List
% delta_tau(OL) = NaN;
for ind=1:FileNum
%        if isnan(delta_tau(ind))
%               continue;
%        end
       % Read Data
%        FileName = ['data_S1_' num2str(ind) '.txt'];
       FileName = ['data_S1_0122.txt'];
       MeasuredData = load(FileName) ;
       Pos     = MeasuredData(:, 1);
       Vel     = MeasuredData(:, 2);
       PosCmd  = MeasuredData(:, 3);
       VelCmd  = MeasuredData(:, 4);
       TorCtrl = MeasuredData(:, 5);

       % Show Error
       PosError = PosCmd - Pos;
       y = lowp(Vel(:,1),10,100,0.1,20,1000);
       VelError = VelCmd - y;
       TorCtrl = lowp(TorCtrl(:,1),1,100,0.1,20,1000); % LP Filter ¶q´ú Tfb

       norm_PosError = norm(PosError) / norm(PosCmd);
       norm_VelError =  norm(VelError) / norm(VelCmd);
       fprintf(1, '%s: \n', FileName)
       fprintf(1, 'norm_PosError = %f (rad)\n', norm_PosError);
       fprintf(1, 'norm_VelError = %f (rad/s)\n\n', norm_VelError);

       % Time Settings
       samp_T = 0.001;
       tf = (size(MeasuredData, 1)-1) * samp_T;
       T = 0:samp_T:tf;

       % Calculate delta_tau
       cut1 = 500;
       cut2 = 2000;
       cut3 = 3500;
       f_up  = fit(VelCmd(cut1:cut2), TorCtrl(cut1:cut2),'poly1');
       f_low = fit(VelCmd(cut2+1:cut3), TorCtrl(cut2+1:cut3),'poly1');
       mid_vel = (VelCmd(cut1) + VelCmd(cut2))/2;
       delta_tau(ind) = (f_up.p1 - f_low.p1)*mid_vel + f_up.p2 - f_low.p2;
       
       % Plot
       figure('Name', FileName, ...
              'NumberTitle', 'off', ...
              'WindowState','maximized')
       subplot(3,1,1)
       plot(VelCmd(cut1:cut2), TorCtrl(cut1:cut2),'.', ...
              VelCmd(cut2+1:cut3), TorCtrl(cut2+1:cut3),'.');
       hold on
       plot(f_up, 'b-')
       hold on
       plot(f_low)
       xline(mid_vel,'LineWidth',2);
       legend('+Acc', '-Acc', 'Up-fitted RHS curve', 'Low-fitted RHS curve', ...
              'Location','best',legendArg{:})
       xlabel('Velocity Command (rad/s)',legendArg{:})
       ylabel('Torque Control Signal ($N \cdot m$)',legendArg{:})
       grid on

       subplot(3,1,2)
       plot(VelCmd, TorCtrl);
       grid on
       
       subplot(3,1,3)
       plot(T, Vel, T, VelCmd, T, Pos, T, PosCmd);
       grid on
end
res = mean(delta_tau, 'omitnan')
AccMax = 30;
J = res/AccMax/2
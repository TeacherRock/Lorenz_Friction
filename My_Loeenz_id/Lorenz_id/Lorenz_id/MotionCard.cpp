...
//可用Function
/*
1. void MotionCard_OpenCard()
用途 : 開啟軸卡
輸入 : 無
輸出 : 無

2. void MotionCard_CloseCard()
用途 : 關閉軸卡
輸入 : 無
輸出 : 無

3. void MotionCard_ChangeTimer(TMRISR Timer)
用途 : 切換中斷 ???
輸入 : ???
輸出 : 無                    

4. void MotionCard_ServoOn()
用途 : Servo On
輸入 : 無
輸出 : 無

5. void MotionCard_Serco_Off()
用途 : Servo Off
輸入 : 無
輸出 : 無

6. void MotionCard_Encoder(double(&JointEncRad))
用途 : 編碼器設定
輸入 : double(&JointEncRad)
輸出 : 更改輸入值(位置)

7. void MotionCard_DAC(double(&JointTorCtrl))
用途 : 將"控制轉矩命令"轉成"馬達轉矩命令"(控制訊號??)
輸入 : double(&JointTorCtrl)
輸出 : 設定輸出電壓

8. void MotionCard_DAC(double(&JointTorCtrl), double(&Voltage))
用途 : 將"控制轉矩命令, 電壓??"轉成"馬達轉矩命令"(電壓??)
輸入 : double(&JointTorCtrl), double(&Voltage) ??
輸出 : 設定輸出電壓
 
9. void MotionCard_ADC(float(&ADCVoltage)[8])
讀取ADC電壓
輸入 : 無
輸出 : 無
